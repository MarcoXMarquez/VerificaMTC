...
public class UserLobbyActivity extends AppCompatActivity {
    ...
    private void loadUserData() {
        ...
        Cursor userCursor = dbHelper.getReadableDatabase().query(
                ...
                new String[]{DatabaseScheme.COLUMN_NAMES, DatabaseScheme.COLUMN_LASTNAMES},
        ...
        if (userCursor != null && userCursor.moveToFirst()) {
            String firstName = userCursor.getString(userCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_NAMES));
            String lastName = userCursor.getString(userCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_LASTNAMES));
        ...
        if (vehicleCursor != null && vehicleCursor.moveToFirst()) {
            String brand = vehicleCursor.getString(vehicleCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_BRAND));
            String plate = vehicleCursor.getString(vehicleCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_PLATE));
        ...
        if (statusCursor != null && statusCursor.moveToFirst()) {
            int hasPaid = statusCursor.getInt(statusCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_HAS_PAID));
            int writtenPassed = statusCursor.getInt(statusCursor.getColumnIndexOrThrow(DatabaseScheme.COLUMN_WRITTEN_EXAM_PASSED));
   ...
   /* public void goToExam(View view) {
    ...
    public void goToVehicleData(View view) {
        Intent intent = new Intent(this, VehicleDataActivity.class);
        intent.putExtra("USER_ID", userId);
        startActivity(intent);
    }
